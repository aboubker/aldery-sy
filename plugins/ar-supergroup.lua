...
local function check_member_super(cb_extra, success, result)
  ...
  if success == 0 then
	send_large_msg(receiver, "لا تلعب بس المطور بيقدر 🖕🏻😊")
  end
...
local function demote_admin(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..' 😠✋🏻 لا تصرعني هوا عضو بلفعل 👿.')
  end
...
local function promote2(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..' 😠✋🏻 لا تصرعني هوا عضو بلفعل 👿.')
  end
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' رفعوك ادمن 😉 شدَ حالك شوي💪.')
end
...
local function demote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' ياحرام نزلو من الادمنية😂 لا تبكي ياروحي انتا 😂😢.')
end
...
function get_message_callback(extra, success, result)
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "❌ لا تلعب بكيفك مافيك تطرد الادمن او الادارة 🖕🏻😎")
    end
	...
	elseif get_cmd == "رفع اداري" then
		...
		if result.from.username then
			text = "@"..result.from.username.."شو بدك أحسن  😍من هيك رفعوك أدارة 😉💪🏻"
		else
			text = "[ "..user_id.." ]شو بدك أحسن  😍من هيك رفعوك أدارة 😉💪🏻"
		end
	...
	elseif get_cmd == "تنزيل اداري" then
		...
		if is_admin2(result.from.peer_id) then
			return send_large_msg(channel_id, "لا تلعب بكيفك مافيك تطرد الادمن او الادارة 🖕🏻😎")
		end
		channel_demote(channel_id, "user#id"..user_id, ok_cb, false)
		if result.from.username then
			text = "@"..result.from.username.."ياحرام شالو 😢 من الادارة لا تبكي ياروحي 😹👻"
		else
			text = "[ "..user_id.." ] ياحرام شالو 😢 من الادارة لا تبكي ياروحي 😹👻"
		end
	...
	elseif get_cmd == "رفع المدير" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "@"..result.from.username.." [ "..result.from.peer_id.." ]شو بدك أحسن✔️ من هيك👍 تم رفعك أدمن 💔"
			else
				text = "[ "..result.from.peer_id.." ] شو بدك أحسن✔️ من هيك👍 تم رفعك أدمن 💔"
			end
			send_large_msg(channel_id, text)
		end
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			...
			send_large_msg(receiver, "["..user_id.."] راح الكتم😉❤️  روح دردشة ونتبه💔")
		elseif is_admin1(msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] ✋🏻راحت عليك تم كتمك 🤐")
		end
	end
end
...
local function cb_user_info(extra, success, result)
	...
	if get_cmd == "تنزيل اداري" then
		if is_admin2(result.peer_id) then
			return send_large_msg(receiver, "لا تلعب بكيفك مافيك تطرد الادمن او الادارة 🖕🏻😎")
		end
		local user_id = "user#id"..result.peer_id
		channel_demote(receiver, user_id, ok_cb, false)
		if result.username then
			text = "@"..result.username.." ياحرام نزلو من الادمنية😂 لا تبكي ياروحي انتا 😂😢"
			send_large_msg(receiver, text)
		else
			text = "[ "..result.peer_id.." ] ياحرام نزلو من الادمنية😂 لا تبكي ياروحي انتا 😂😢"
			send_large_msg(receiver, text)
		end
	elseif get_cmd == "رفع ادمن" then
		if result.username then
...
elseif get_cmd == "رفع اداري" then
   for k,v in pairs(result) do
    ...
    if vusername == member or vpeer_id == memberid then
      ...
      channel_set_admin(channel_id, user_id, ok_cb, false)
      if v.username then
        text = "@"..v.username.." ["..v.peer_id.."] شو بدك أحسن  😍من هيك رفعوك أدارة 😉💪🏻"
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]")
      else
        text = "["..v.peer_id.."] شو بدك أحسن  😍من هيك رفعوك أدارة 😉💪🏻"
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = member_username.." ["..v.peer_id.."] شو بدك أحسن✔️ من هيك👍 تم رفعك مدير 💔"
				else
					text = "["..v.peer_id.."] شو بدك أحسن✔️ من هيك👍 تم رفعك مدير 💔"
				end
			end
		elseif memberid and vusername ~= member and vpeer_id ~= memberid then
			...
			if group_owner then
				...
				text = "["..memberid.."] شو بدك أحسن✔️ من هيك👍 تم رفعك مدير 💔"
			end
		end
	end
 end
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'رفع ادمن' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "لا تلعب بس الادمن و المدير فيهم 🖕🏻😊"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'رفع ادمن',
		...
		if matches[1] == 'مسح' then
			...
			if not is_momod(msg) then
				return "لا تلعب بس المدير يحقلو 🖕🏻😊"
			end
